...
export interface TeamAccountCreateArgs {
    name: string;
    mail: string;
    mobile: string;
}
...
export const TeamAccountCreate = async ({ name, mail, mobile }: TeamAccountCreateArgs): Promise<any> => {
    try {
        ...
        const datx = {
            data: {
                name, // team ID
                mail, // { name, mail, mobile }
                mobile, // e.g. 'admin', 'viewer', etc.
            },
            srvc: "988b9aee-f02a-411c-957d-02ef420586a2"
        };

        const args = [{ body: JSON.stringify(datx) }];
        const result = await realmUser.functions["business_account_create"](...args);
        return result;
    } catch (error) {
